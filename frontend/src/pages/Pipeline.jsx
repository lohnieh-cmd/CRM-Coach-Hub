...
import { Kanban, Plus, Mountains } from "@phosphor-icons/react";
    ...
    <div>
        ...
        actions={
          <>
            <div className="flex rounded-lg border border-[#283341] overflow-hidden">
              <button
                className={`px-3 py-2 text-sm ${view === "kanban" ? "bg-[#212a36] text-white" : "text-[#94a3b8]"}`}
              ...
              <button
                className={`px-3 py-2 text-sm ${view === "list" ? "bg-[#212a36] text-white" : "text-[#94a3b8]"}`}
      ...
      {stages.length === 0 ? (
        <div className="p-8">
          <Empty
            title="No stages yet"
            subtitle="Apply a coaching template to start with a ready pipeline (Basecamp → Summit)."
            icon={Mountains}
            cta={
              <a className="btn btn-primary" href="/templates" data-testid="goto-templates">
                Browse Templates
              </a>
            }
          />
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <div className="elev" title={`${d.probability}% likely`}>
                              {[0, 1, 2, 3, 4].map((i) => (
                                <span key={i} className={i < bars ? "on" : ""} />
                              ))}
                            </div>
                          </div>
                        </div>
      ...
      <NewDealModal open={showNew} onClose={() => setShowNew(false)} stages={stages} contacts={contacts} companies={companies} onSaved={load} />
    </div>
  );
}

function NewDealModal({ open, onClose, stages, contacts, companies, onSaved }) {
  const [form, setForm] = useState({ title: "", value: 0, currency: "USD", probability: 10 });
  useEffect(() => {
    if (open && stages.length) setForm((f) => ({ ...f, pipeline_stage_id: stages[0].id, probability: stages[0].probability }));
  }, [open, stages]);
  ...
  const save = async () => {
    try {
      ...
      onSaved();
      onClose();
      setForm({ title: "", value: 0, currency: "USD", probability: 10 });
    } catch (e) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create Deal">
      <div className="space-y-4" data-testid="new-deal-form">
        <Field label="Title">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Stage">
            <select className="select" value={form.pipeline_stage_id || ""} onChange={(e) => {
              ...
              setForm({ ...form, pipeline_stage_id: e.target.value, probability: s?.probability || 10 });
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button className="btn btn-primary" onClick={save} data-testid="deal-save">Create Deal</button>
        </div>
      </div>
    </Modal>
  );
